...
There are a varierty of signal processing techniques that can be applied to time series data. For example, the trend components can be extracted from the time series by transforming the signal into the frequency domain, and filtering out non dominant frequencies. This tends to work well for finding the overall trend of the time series. A trend is will be defined as an periodic patterns that have less than or equal to 2 cycles in the time series. These methods don't work so well when the data is non-stationary or have seasonal components or periodic patterns that have more than 2 cycles in the time series.     http://www.datasciencecentral.com/profiles/blogs/how-we-combined-different-methods-to-create-advanced-time-series.
